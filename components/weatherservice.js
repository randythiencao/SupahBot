...
WeatherService.prototype.getWeather = function(city, message) {
  if (city.length > 1) {
    ...
    this.getWeatherForCity('Arlington', message);
  }
}

WeatherService.prototype.getWeatherForCity = function(city, message) {
  var vm = this;
  request('http://api.openweathermap.org/data/2.5/weather?q=' + city + '&APPID=' + vm.apikey + '&units=imperial', (error, response, body) => {
