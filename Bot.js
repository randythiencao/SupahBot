...
var commands = {
  ...
  '!weather': {
    execute: getWeather,
    description: 'get current weather for a city, defaults to Arlington'
  },
  '!roll': {
    execute: roll,
  ...
  '!words': {
    ...
    description: 'most popular words for a user, defaults to your username'
  },
  '!queue': {
    execute: doQueue,
  ...
  if (Object.keys(commands).length > 1) {
    ...
    for (var command in commands) {
      if (command != '!help') {
        ...
        toReturn += command+"\n\t\t"+data.description + getAvailableCommandAsText(data)+"\n";
		//toReturn = '\`\`\`diff\n' + '-'+command +'\n\`\`\`'+ '\n\t'+data.description + getAvailableCommandAsText(data) + '\n';
      }
    }
	toReturn += "\n```\n";
